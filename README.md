# non magical csi - v2.0

> no magical radar

Firmware for the M5Stack Cardputer that reads raw WiFi Channel State Information (CSI) from the ESP32-S3 radio, visualises it in real time, and collects labelled training data for on-device ML models. Built for studying what CSI actually is. For demo and educational purposes.
...
This firmware captures those measurements, draws them live on screen, runs a motion detector on top, and (v2.0) guides you through structured data-collection sessions to build ML training datasets.
...
5. CSV is written to SD card at `/non-magical-csi/<name>_<proc>.csv`
...
**zone** (~70 s) - mark 4 positions A-D on the floor and stand at each. Labels: `zone_a`, `zone_b`, `zone_c`, `zone_d`, `absent`.
